...
const FUNNEL_COLUMNS: { id: LeadStatus; title: string; color: string }[] = [
  ...
  { id: "qualificado", title: "Qualificado", color: "border-purple-500 text-purple-400 bg-purple-500/10" },
  { id: "apresentado", title: "Imóvel Apresentado", color: "border-amber-500 text-amber-400 bg-amber-500/10" },
  { id: "visita", title: "Visita Agendada", color: "border-teal-500 text-teal-400 bg-teal-500/10" },
  { id: "proposta", title: "Proposta", color: "border-orange-500 text-orange-400 bg-orange-500/10" },
  { id: "negociacao", title: "Negociação", color: "border-rose-500 text-rose-400 bg-rose-500/10" },
  { id: "fechado", title: "Fechado", color: "border-emerald-500 text-emerald-400 bg-emerald-500/10" },
];
...
export const LeadsFunnelModule: React.FC<LeadsFunnelModuleProps> = ({
  ...
  const filteredLeads = leads.filter((lead) => {
    const matchesSearch =
      ...
      (lead.interestedInPropertyTitle && lead.interestedInPropertyTitle.toLowerCase().includes(searchQuery.toLowerCase()));
                  ...
                  <div className="p-3.5 border-b border-slate-800/80 bg-slate-950/40 rounded-t-2xl">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className={`w-2.5 h-2.5 rounded-full ${column.color.split(" ")[0]} border-2`} />
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 font-medium">
                      Total: <strong className="text-amber-300">{formatCurrency(columnTotalBudget)}</strong>
                    </p>
                  </div>
                      ...
                      columnLeads.map((lead) => (
                        ...
                        >
                          ...
                          <div className="mt-2 pt-2 border-t border-slate-750 flex items-center justify-between text-[11px]">
                            <div>
                              <span className="text-[10px] text-slate-400 block">Orçamento Est.</span>
                              <span className="font-bold text-amber-300">{formatCurrency(lead.estimatedBudget)}</span>
                            </div>
                            <div className="text-right">
                              <span className="text-[10px] text-slate-400 block">Corretor</span>
                              <span className="text-slate-200 font-medium">{lead.brokerName.split(" ")[0]}</span>
                            </div>
                          </div>
                ...
                {filteredLeads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-slate-850/50 transition-colors">
                    <td className="py-3 px-4">
                      ...
                      <div className="text-[11px] text-slate-400 capitalize">{lead.type} • CPF: {lead.document}</div>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      ...
                      {lead.campaign && (
                        <div className="text-[10px] text-slate-400 mt-1 truncate max-w-[180px]">{lead.campaign}</div>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <div className="font-bold text-amber-300">{formatCurrency(lead.estimatedBudget)}</div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 max-w-md w-full shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-300">
              Você está arquivando o lead <strong className="text-white">{leadToLose.name}</strong>. Conforme as regras do CRM SENA 2026, informe o motivo para alimentar os relatórios de perda:
            </p>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="Preço acima do orçamento / Sem condição de pagamento">Preço acima do orçamento / Sem condição de pagamento</option>
                <option value="Optou por imóvel concorrente em outra imobiliária">Optou por imóvel concorrente em outra imobiliária</option>
                <option value="Crédito ou financiamento bancário reprovado">Crédito ou financiamento bancário reprovado</option>
                <option value="Desistência temporária da compra/locação">Desistência temporária da compra/locação</option>
                <option value="Localização ou metragem inadequada">Localização ou metragem inadequada</option>
                <option value="Falta de retorno após múltiplas tentativas">Falta de retorno após múltiplas tentativas</option>
              </select>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-xl w-full shadow-2xl space-y-4 my-8">
            ...
            <form onSubmit={handleCreateLead} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Corretor Responsável</label>
                  <select
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Campanha Específica</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">
                  Orçamento Estimado (R$): <strong className="text-amber-400">{formatCurrency(newLeadBudget)}</strong>
                </label>
                <input
                  type="range"
                  min="200000"
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Observações Iniciais</label>
                <textarea
                  rows={2}
                  value={newLeadNotes}
