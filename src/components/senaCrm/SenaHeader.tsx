...
export const SenaHeader: React.FC<SenaHeaderProps> = ({
  ...
  const getTabTitle = (tab: SenaTab): { title: string; subtitle: string } => {
    switch (tab) {
      case "dashboard":
        return { title: "Painel Executivo & Indicadores", subtitle: "Consolidado Geral: Vendas, Locações e Loteamentos" };
      case "leads":
        return { title: "Gestão de Leads & Prospecção", subtitle: "Rastreamento completo de origens e campanhas" };
      case "funil":
        return { title: "Funil Comercial de Vendas (Kanban)", subtitle: "Acompanhamento dinâmico de etapas até o fechamento" };
      case "clientes":
        return { title: "Clientes & Perfil de Interesse", subtitle: "Base de compradores, locatários, investidores e matching inteligente" };
      case "visitas":
        return { title: "Agenda & Controle de Visitas", subtitle: "Agendamentos, visitas do dia e feedbacks de clientes" };
      case "propostas":
        return { title: "Propostas & Histórico de Negociações", subtitle: "Valores propostos, contrapropostas e aprovações" };
      case "imoveis":
        return { title: "Carteira Completa de Imóveis", subtitle: "Catálogo de alto padrão com filtros e status" };
      case "proprietarios":
        return { title: "Base de Proprietários & Locadores", subtitle: "Gestão de patrimônio e relacionamento" };
      case "vendas":
        return { title: "Fechamentos de Venda Concluídos", subtitle: "Contratos assinados, VGV realizado e comissões" };
      case "locacoes-contratos":
        return { title: "Contratos de Locação Administrados", subtitle: "Garantias locatícias, vencimentos e reajustes" };
      case "locacoes-repasses":
        return { title: "Recebimentos & Repasses aos Proprietários", subtitle: "Cálculo automático: Aluguel - Taxa adm - Despesas = Líquido" };
      case "locacoes-vistorias":
        return { title: "Vistorias Técnicas (Entrada/Saída)", subtitle: "Laudos fotográficos e aprovações digitais" };
      case "locacoes-manutencoes":
        return { title: "Chamados de Manutenção & Reparos", subtitle: "Prestadores de serviços e autorizações de despesas" };
      case "loteamentos-empreendimentos":
        return { title: "Empreendimentos & Lançamentos", subtitle: "Loteamentos fechados e urbanismo sustentável" };
      case "loteamentos-espelho":
        return { title: "Espelho de Vendas Visual Interativo", subtitle: "Mapa em tempo real das quadras e lotes por status" };
      case "loteamentos-simulador":
        return { title: "Simulador Financeiro de Lotes", subtitle: "Cálculo de entrada, parcelamento direto e saldo financiado" };
      case "loteamentos-reservas":
        return { title: "Reservas & Bloqueios de Lotes", subtitle: "Gestão de reservas temporárias com bloqueio antifuro" };
      case "equipe-corretores":
        return { title: "Equipe de Corretores & Performance", subtitle: "CRECI, VGV acumulado, conversão e metas" };
      case "equipe-comissoes":
        return { title: "Distribuição e Divisão de Comissões", subtitle: "Cálculo transparente entre imobiliária, gerente, captador e corretor" };
      case "equipe-ranking":
        return { title: "Ranking Comercial & Premiações", subtitle: "Destaques do mês e trimestre em VGV gerado" };
      case "sistema-usuarios":
        return { title: "Gestão de Usuários & Níveis de Acesso", subtitle: "Permissões de corretores, gerentes e diretores" };
      case "sistema-configuracoes":
        return { title: "Configurações da Imobiliária SENA", subtitle: "Parâmetros operacionais, índices e percentuais padrão" };
      default:
        ...
        <div className="flex items-center gap-2 lg:gap-3 shrink-0">
          ...
          <div className="hidden xl:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/80 border border-slate-700 text-xs text-slate-300">
            ...
            >
              ...
              <option value="Plantão Loteamentos Itu / Sorocaba" className="bg-slate-900 text-white">
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 p-3 text-xs space-y-2">
                <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                  ...
                  <span className="text-[10px] text-amber-400 font-semibold bg-amber-500/10 px-2 py-0.5 rounded">3 Novas</span>
                </div>
                <div className="space-y-2">
                  <div className="p-2 rounded-lg bg-slate-800/60 border border-slate-700/60">
                    ...
                    <p className="text-slate-300 text-[11px]">Guilherme Leite propôs R$ 4.600.000 no imóvel SENA-801.</p>
                  ...
                  <div className="p-2 rounded-lg bg-slate-800/60 border border-slate-700/60">
                    ...
                    <p className="text-slate-300 text-[11px]">Lote 04 - Quadra B (Reserva dos Ipês) reservado.</p>
                    <span className="text-[10px] text-slate-400">Há 1 hora</span>
                  </div>
                  <div className="p-2 rounded-lg bg-slate-800/60 border border-slate-700/60">
                    <p className="font-semibold text-blue-300">Aluguel Confirmado</p>
                    <p className="text-slate-300 text-[11px]">Venture Partners efetuou pagamento de R$ 45.000.</p>
