  ...
  const calculateMatchingProperties = (lead?: Lead) => {
    if (!lead || !lead.interestProfile) {
      ...
      return properties
        .map((prop) => {
          ...
          return { property: prop, score: Math.min(score, 98), reasons: ["Faixa de valor compatível"] };
        })
    ...
    return properties
      .map((prop) => {
        ...
        // Objective check
        if (
          (profile.objective === "compra" && (prop.purpose === "venda" || prop.purpose === "ambos")) ||
          (profile.objective === "locacao" && (prop.purpose === "locacao" || prop.purpose === "ambos"))
        ) {
          score += 15;
        ...
        // Neighborhood check
        if (profile.preferredNeighborhoods.some((n) => prop.neighborhood.toLowerCase().includes(n.toLowerCase()))) {
          score += 15;
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1.5 flex items-center justify-between text-[11px] text-slate-400">
                    <span>Orçamento: <strong className="text-amber-400">{formatCurrency(client.estimatedBudget)}</strong></span>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-800">
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-extrabold text-white">{currentSelectedLead.name}</h3>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    CPF/CNPJ: {currentSelectedLead.document} • Corretor: {currentSelectedLead.brokerName}
                  </p>
                </div>
              ...
              {currentSelectedLead.interestProfile ? (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 text-xs">
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                    ...
                    <span className="font-bold text-white uppercase">{currentSelectedLead.interestProfile.objective}</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-[10px] text-slate-400 block font-medium">Tipos Desejados</span>
                    <span className="font-bold text-white truncate block">
                      {currentSelectedLead.interestProfile.propertyTypes.join(", ")}
                    </span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-[10px] text-slate-400 block font-medium">Bairros Preferidos</span>
                    <span className="font-bold text-white truncate block">
                      {currentSelectedLead.interestProfile.preferredNeighborhoods.join(", ")}
                    </span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-[10px] text-slate-400 block font-medium">Forma de Pagamento</span>
                ...
                <div className="pt-3 text-xs text-slate-400">
                  Cadastre as preferências detalhadas no perfil do cliente para refinar o matching algorítmico.
                </div>
              )}
            </div>
          )}
