...
export const ReportsModule: React.FC<ReportsModuleProps> = ({
  ...
  const [reportType, setReportType] = useState<"vendas" | "captacoes" | "visitas" | "locacao" | "loteamentos">("vendas");
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-400">Exportação e consolidação de dados operacionais e financeiros</p>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase font-bold">VGV Total Fechado</span>
              <p className="text-xl font-black text-amber-400">{formatCurrency(totalVgvSales)}</p>
            </div>
            <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase font-bold">Honorários Gerados (6%)</span>
              <p className="text-xl font-black text-emerald-400">{formatCurrency(totalCommissionsSales)}</p>
            </div>
            <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase font-bold">Ticket Médio de Venda</span>
              <p className="text-xl font-black text-white">{formatCurrency(totalVgvSales / Math.max(1, sales.length))}</p>
            </div>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xs p-5">
            <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-3">Tabela Detalhada de Fechamentos</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800 text-slate-200">
                  {sales.map((s) => (
                    <tr key={s.id}>
                      <td className="py-3 px-3 font-mono text-amber-400 font-bold">{s.code} ({s.saleDate})</td>
                      <td className="py-3 px-3 font-medium text-white">{s.propertyTitle}</td>
                      <td className="py-3 px-3 text-slate-300">{s.clientName}</td>
                      <td className="py-3 px-3 font-black text-amber-400">{formatCurrency(s.finalSalePrice)}</td>
                      <td className="py-3 px-3 font-bold text-emerald-400">{formatCurrency(s.commissionTotal)}</td>
      ...
      {reportType === "captacoes" && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
          <h3 className="text-sm font-bold text-white">Captações Ativas no Portfólio ({properties.length} imóveis)</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-950/80 text-slate-400 font-semibold border-b border-slate-800 uppercase tracking-wider text-[10px]">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800 text-slate-200">
                {properties.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td className="py-3 px-3 font-semibold text-white">{p.title} ({p.neighborhood})</td>
                    <td className="py-3 px-3 text-slate-300">{p.ownerName}</td>
                    <td className="py-3 px-3 text-emerald-400 font-medium">{p.brokerCaptatorName}</td>
      ...
      {reportType === "visitas" && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
          <div className="flex justify-between items-center pb-2 border-b border-slate-800">
            <h3 className="text-sm font-bold text-white">Relatório de Visitação e Índice de Conversão</h3>
            <span className="text-xs text-amber-400 font-bold">Taxa de Conversão em Proposta: 60%</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-800 text-slate-200">
                {visits.map((v) => (
                  <tr key={v.id}>
                    <td className="py-3 px-3 font-mono">{v.date} às {v.time}</td>
                    ...
                    <td className="py-3 px-3 text-amber-300 italic">{v.clientImpression || "Sem laudo"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800">
              <span className="text-[10px] text-slate-400 uppercase font-bold block">Aluguel Bruto Arrecadado</span>
              <p className="text-xl font-black text-white mt-1">{formatCurrency(totalRentCollected)}</p>
            </div>
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800">
              <span className="text-[10px] text-slate-400 uppercase font-bold block">Taxa de Adm Recebida pela Imobiliária</span>
              <p className="text-xl font-black text-emerald-400 mt-1">{formatCurrency(totalRentAdminFee)}</p>
            </div>
          </div>
          ...
          <div className="overflow-x-auto pt-2">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-800 text-slate-200">
                {rentalPayouts.map((p) => (
                  <tr key={p.id}>
                    <td className="py-3 px-3 font-mono font-bold text-amber-400">{p.contractNumber}</td>
                    <td className="py-3 px-3 font-medium text-white">{p.ownerName}</td>
                    <td className="py-3 px-3 text-white font-bold">{formatCurrency(p.grossRentReceived)}</td>
                    <td className="py-3 px-3 text-rose-400 font-medium">− {formatCurrency(p.adminFeeDeduction)}</td>
                    <td className="py-3 px-3 text-emerald-400 font-black">{formatCurrency(p.netOwnerPayout)}</td>
                    <td className="py-3 px-3">{p.status}</td>
                  </tr>
                ))}
              </tbody>
      ...
      {reportType === "loteamentos" && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
          <h3 className="text-sm font-bold text-white">Desempenho Geral de Loteamentos & VGV Urbanístico</h3>
          <div className="space-y-4">
            {developments.map((d) => {
              ...
              return (
                <div key={d.id} className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
                  <div className="flex justify-between items-center">
                    <h4 className="font-bold text-white">{d.name} ({d.location})</h4>
                    <span className="text-sm font-black text-amber-400">VGV: {formatCurrency(d.totalVgv)}</span>
                  </div>
                  <div className="flex gap-4 text-xs text-slate-300">
                    <span>Total de Lotes: <strong>{d.totalLots}</strong></span>
                    <span>Lotes Vendidos: <strong className="text-emerald-400">{soldLotsCount}</strong></span>
                    <span>Disponíveis: <strong className="text-blue-400">{availableLotsCount}</strong></span>
                    <span>VGV Realizado: <strong className="text-amber-300">{formatCurrency(d.soldVgv)}</strong></span>
                  </div>
                </div>
              );
            })}
