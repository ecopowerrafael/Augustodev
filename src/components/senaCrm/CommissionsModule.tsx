...
export const CommissionsModule: React.FC<CommissionsModuleProps> = ({ commissions }) => {
  ...
  const brokersValSum = commissions.reduce((acc, c) => acc + c.captatorShareValue + c.attendantBrokerShareValue, 0);
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xs">
        <div>
          ...
          <h2 className="text-lg font-bold text-white">Módulo de Divisão Transparente de Comissões</h2>
          <p className="text-xs text-slate-400">
            Regras de split entre Imobiliária (40%), Gerência (10%), Captador (25%) e Corretor de Atendimento (25%)
          </p>
        </div>

        <div className="flex items-center gap-3">
          <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-right">
            <span className="text-[10px] text-slate-400 block uppercase font-bold">Total Distribuído</span>
            <span className="text-base font-black text-emerald-400">{formatCurrency(totalCommissionValSum)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-850 to-slate-900 border border-amber-500/30 rounded-2xl p-5 lg:p-6 shadow-md space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Simulador Interativo de Split de Honorários</h3>
              <p className="text-xs text-slate-400">Ajuste o valor da venda e veja o cálculo em tempo real</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div>
              <div className="flex justify-between text-xs font-semibold mb-1">
                ...
                <span className="text-amber-400 font-bold">{simTotalCommissionPct.toFixed(1)}%</span>
              </div>
              <input
                type="range"
                min="4"
            ...
            <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800">
              <div className="flex justify-between items-center text-xs">
                ...
                <span className="text-lg font-black text-emerald-400">{formatCurrency(simTotalCommissionVal)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 font-bold uppercase">Imobiliária SENA (40%)</span>
              <p className="text-sm font-black text-amber-300">{formatCurrency(simAgencyVal)}</p>
              <span className="text-[10px] text-slate-500 block">Estrutura, Marketing e Jurídico</span>
            </div>

            <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 font-bold uppercase">Gerência Comercial (10%)</span>
            ...
            <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 font-bold uppercase">Corretor Captador (25%)</span>
              <p className="text-sm font-black text-emerald-300">{formatCurrency(simCaptatorVal)}</p>
              <span className="text-[10px] text-slate-500 block">Captação exclusiva do imóvel</span>
            </div>

            <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
              <span className="text-[10px] text-slate-400 font-bold uppercase">Corretor Atendimento (25%)</span>
              <p className="text-sm font-black text-purple-300">{formatCurrency(simAttendantVal)}</p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xs space-y-3 p-5">
        <div className="flex items-center justify-between pb-2 border-b border-slate-800">
          <div>
            <h3 className="text-sm font-bold text-white">Relatório de Comissões e Repasses Cadastrados</h3>
              ...
              {commissions.map((c, idx) => (
                <tr key={idx} className="hover:bg-slate-850/50 transition-colors">
                  <td className="py-3 px-3">
                    ...
                    <span className="text-[11px] text-slate-400 truncate block max-w-[200px]">{c.propertyTitle}</span>
                  </td>
                  <td className="py-3 px-3 font-semibold text-slate-300">{formatCurrency(c.saleValue)}</td>
                  <td className="py-3 px-3 font-bold text-amber-400">{formatCurrency(c.totalCommissionValue)}</td>
                  <td className="py-3 px-3 text-slate-300">{formatCurrency(c.agencyShareValue)} ({c.agencySharePercentage}%)</td>
                  <td className="py-3 px-3 text-slate-300">{formatCurrency(c.managerShareValue)} ({c.managerSharePercentage}%)</td>
                  <td className="py-3 px-3 text-emerald-400 font-semibold">{formatCurrency(c.captatorShareValue)} ({c.captatorSharePercentage}%)</td>
                  <td className="py-3 px-3 text-purple-400 font-semibold">{formatCurrency(c.attendantBrokerShareValue)} ({c.attendantBrokerSharePercentage}%)</td>
                  <td className="py-3 px-3">
                    <span
