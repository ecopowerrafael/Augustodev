...
export const RentalsModule: React.FC<RentalsModuleProps> = ({
  ...
  const [subTab, setSubTab] = useState<"funil" | "contratos" | "repasses" | "vistorias" | "manutencoes">(
  ...
  const totalMonthlyRentAdministered = rentalContracts.reduce((acc, c) => acc + c.monthlyRentalValue, 0);
  const totalAdminFeesMonthly = rentalContracts.reduce((acc, c) => acc + (c.monthlyRentalValue * c.adminFeePercentage) / 100, 0);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Carteira ativa: <strong className="text-amber-300">{formatCurrency(totalMonthlyRentAdministered)}/mês</strong> sob gestão • Taxa adm: <strong className="text-emerald-400">{formatCurrency(totalAdminFeesMonthly)}/mês</strong>
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 bg-slate-950 p-1.5 rounded-xl border border-slate-800 self-start md:self-auto">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              subTab === "contratos" ? "bg-amber-500 text-slate-950 font-bold" : "text-slate-400 hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              subTab === "repasses" ? "bg-amber-500 text-slate-950 font-bold" : "text-slate-400 hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              subTab === "vistorias" ? "bg-amber-500 text-slate-950 font-bold" : "text-slate-400 hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              subTab === "manutencoes" ? "bg-amber-500 text-slate-950 font-bold" : "text-slate-400 hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              subTab === "funil" ? "bg-amber-500 text-slate-950 font-bold" : "text-slate-400 hover:text-white"
            }`}
          >
            Funil de Locação
          </button>
      ...
      {subTab === "contratos" && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xs space-y-4 p-5">
          <div className="flex items-center justify-between pb-2 border-b border-slate-800">
            <div>
              <h3 className="text-sm font-bold text-white">Carteira de Contratos de Locação Administrados</h3>
              <p className="text-xs text-slate-400">Controle de vencimentos, taxas administrativas e garantias</p>
            </div>
          </div>
            ...
            {rentalContracts.map((contract) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase border ${getPaymentBadge(contract.paymentStatus)}`}>
                ...
                <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-800 space-y-1 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-300 font-semibold">{contract.readjustmentIndex} (Próx: {contract.nextReadjustmentDate})</span>
                  </div>
                </div>

                <div className="pt-2 border-t border-slate-800 flex items-center justify-between text-xs">
                  <div>
                    <span className="text-[10px] text-slate-400 block">Aluguel Bruto (Venc. dia {contract.dueDay})</span>
                    <span className="text-sm font-black text-amber-400">{formatCurrency(contract.monthlyRentalValue)}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] text-slate-400 block">Taxa Adm ({contract.adminFeePercentage}%)</span>
                    <span className="text-xs font-bold text-emerald-400">
                      {formatCurrency((contract.monthlyRentalValue * contract.adminFeePercentage) / 100)}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-slate-900 to-slate-850 border border-slate-800 rounded-2xl p-5 shadow-xs">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-white bg-slate-950 p-3 rounded-xl border border-slate-800">
              ...
              <span className="text-slate-500 font-black">=</span>
              <span className="text-emerald-400 font-black text-sm">Repasse Líquido ao Proprietário</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xs p-5 space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              <div>
                <h3 className="text-sm font-bold text-white">Extrato de Recebimentos & Repasses (Competência 08/2026)</h3>
                <p className="text-xs text-slate-400">Demonstrativo financeiro discriminado por imóvel</p>
              </div>
            </div>
                  ...
                  {rentalPayouts.map((payout) => (
                    <tr key={payout.id} className="hover:bg-slate-850/50 transition-colors">
                      <td className="py-3 px-3">
                        <span className="font-mono text-amber-400 font-bold">{payout.contractNumber}</span>
                        <span className="block text-slate-300 font-semibold truncate max-w-[200px]">{payout.propertyTitle}</span>
                      </td>
                      <td className="py-3 px-3 font-medium text-white">{payout.ownerName}</td>
                      <td className="py-3 px-3 font-bold text-white">{formatCurrency(payout.grossRentReceived)}</td>
                      <td className="py-3 px-3 text-rose-400 font-medium">− {formatCurrency(payout.adminFeeDeduction)}</td>
                      <td className="py-3 px-3 text-orange-400 font-medium">
                        {payout.authorizedExpensesDeduction > 0 ? `− ${formatCurrency(payout.authorizedExpensesDeduction)}` : "R$ 0"}
                      </td>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
          <div className="flex items-center justify-between pb-2 border-b border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-400">Laudos fotográficos de Entrada, Periódica e Saída</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {inspections.map((insp) => (
              <div key={insp.id} className="p-4 rounded-xl bg-slate-850 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">Vistoriador: {insp.inspectorName} • Data: {insp.date.split("-").reverse().join("/")}</p>
                </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
          <div className="flex items-center justify-between pb-2 border-b border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-400">Orçamentos com prestadores e autorizações do proprietário</p>
            </div>
          </div>

          <div className="space-y-3">
            {maintenances.map((m) => (
              <div key={m.id} className="p-4 rounded-xl bg-slate-850 border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-750 text-[11px]">
                  <span className="text-slate-400">Prestador: <strong className="text-slate-200">{m.serviceProviderName}</strong></span>
                  <span className="text-slate-400">Orçamento: <strong className="text-amber-400">{formatCurrency(m.budgetAmount)}</strong></span>
      ...
      {subTab === "funil" && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
          <div>
            ...
            <p className="text-xs text-slate-400">Passo a passo padrão para locação segura e sem burocracia</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3 pt-2">
            {[
              { step: "01", title: "Lead Inicial", desc: "Captação & Perfil de Interesse", color: "border-blue-500" },
              { step: "02", title: "Visita Agendada", desc: "Apresentação com Corretor", color: "border-indigo-500" },
              { step: "03", title: "Análise Cadastral", desc: "Score & Renda (CredPago/Porto)", color: "border-purple-500" },
              { step: "04", title: "Aprovação & Garantia", desc: "Emissão de Apólice Fiança", color: "border-amber-500" },
              { step: "05", title: "Contrato Digital", desc: "Minuta & Assinatura Eletrônica", color: "border-teal-500" },
              { step: "06", title: "Vistoria Inicial", desc: "Laudo com +80 fotos periciais", color: "border-emerald-500" },
              { step: "07", title: "Entrega das Chaves", desc: "Posse do imóvel & Início da locação", color: "border-yellow-500" },
            ].map((st, sIdx) => (
              <div key={sIdx} className={`p-3 rounded-xl bg-slate-850 border-t-2 ${st.color} border-slate-800 space-y-1 text-xs`}>
                <span className="text-[10px] font-black text-amber-400 font-mono">ETAPA {st.step}</span>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="text-base font-bold text-white">Demonstrativo de Repasse ao Proprietário</h3>
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
                ...
                <p className="text-slate-400">Proprietário Beneficiário: <strong className="text-slate-200">{selectedPayout.ownerName}</strong></p>
                <p className="text-slate-400">Competência: <strong>{selectedPayout.competenceMonth}</strong></p>
              </div>

              <div className="space-y-2 pt-2">
                <div className="flex justify-between text-slate-300">
                  <span>Aluguel Bruto Recebido:</span>
                  <span className="font-bold text-white">{formatCurrency(selectedPayout.grossRentReceived)}</span>
                </div>
