...
export const PropertiesModule: React.FC<PropertiesModuleProps> = ({
  ...
  const [propImageUrl, setPropImageUrl] = useState("https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=800&auto=format&fit=crop&q=80");
            ...
            >
                  ...
                  <div className="pt-2">
                    {property.purpose === "locacao" ? (
                      <div className="text-base font-black text-amber-400">
                        {formatCurrency(property.rentalPrice || 0)} <span className="text-xs font-normal text-slate-400">/mês</span>
                      </div>
                    ) : property.purpose === "ambos" ? (
                      <div>
                        <span className="text-base font-black text-amber-400">{formatCurrency(property.salePrice)}</span>
                        <span className="text-xs text-slate-400 block font-normal">ou {formatCurrency(property.rentalPrice || 0)} /mês</span>
                      </div>
                    ) : (
                      <div className="text-base font-black text-amber-400">
              ...
              <div className="p-3 bg-slate-950/60 border-t border-slate-800 flex items-center justify-between text-xs">
                <div className="flex items-center gap-1.5 text-slate-400 truncate">
                  ...
                  <span className="truncate">Captador: <strong className="text-slate-200">{property.brokerCaptatorName.split(" ")[0]}</strong></span>
                </div>
              ...
              <tbody className="divide-y divide-slate-800 text-slate-200">
                {filteredProperties.map((prop) => (
                  <tr key={prop.id} className="hover:bg-slate-850/50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div>{prop.type}</div>
                      <div className="text-[11px] text-slate-400">{prop.neighborhood}, {prop.city}</div>
                    </td>
                    <td className="py-3 px-4">
                      {prop.purpose === "locacao" ? (
                        <div className="font-bold text-amber-400">{formatCurrency(prop.rentalPrice || 0)} /mês</div>
                      ) : (
                        <div className="font-bold text-amber-400">{formatCurrency(prop.salePrice)}</div>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <div>{prop.totalArea}m² totais ({prop.privateArea}m² priv.)</div>
                      <div className="text-[11px] text-slate-400">{prop.bedrooms} qtos ({prop.suites} suítes) • {prop.parkingSpots} vagas</div>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border ${getStatusBadge(prop.status)}`}>
                        {prop.status}
                      </span>
                    </td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-2xl w-full shadow-2xl space-y-4 my-8">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-400">Inserção de nova captação exclusiva ou padrão</p>
              </div>
              <button
            ...
            <form onSubmit={handleCreateProperty} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="md:col-span-2">
                  <label className="font-semibold text-slate-300 block mb-1">Título do Imóvel *</label>
                  <input
                    type="text"
                    required
                    value={propTitle}
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Finalidade Comercial</label>
                  <select
                    value={propPurpose}
                ...
                {propPurpose !== "locacao" && (
                  <div>
                    <label className="font-semibold text-slate-300 block mb-1">Valor de Venda (R$)</label>
                    <input
                      type="number"
                      value={propSalePrice}
                ...
                {propPurpose !== "venda" && (
                  <div>
                    <label className="font-semibold text-slate-300 block mb-1">Valor de Locação Mensal (R$)</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Bairro / Condomínio *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Nome do Proprietário</label>
                  <input
                    type="text"
                    value={propOwnerName}
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Telefone do Proprietário</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Corretor Captador</label>
                  <select
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Foto Principal (URL)</label>
                  <input
                    type="text"
                    value={propImageUrl}
