    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl space-y-1">
          ...
          <p className="text-2xl font-black text-white">{brokers.filter((b) => b.status === "Ativo").length}</p>
        </div>
        <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl space-y-1">
        ...
        <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl space-y-1">
          <span className="text-xs font-medium text-slate-400">Comissões Geradas aos Corretores</span>
          <p className="text-2xl font-black text-emerald-400">{formatCurrency(totalCommissionsTeam)}</p>
        </div>
      </div>
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-2 text-xs">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-amber-400">{formatCurrency(broker.vgvTotal)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400 font-semibold">Comissão Recebida:</span>
                    <span className="font-bold text-emerald-400">{formatCurrency(broker.commissionEarned)}</span>
                  </div>
                  <div className="flex justify-between items-center">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleCreateBroker} className="space-y-3 text-xs">
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">E-mail Corporativo:</label>
                <input
                  type="email"
                  value={email}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  >
                    ...
                    <option value="Equipe Terra — Loteamentos & Investimentos">Equipe Terra — Loteamentos</option>
                    <option value="Equipe Locações Prime">Equipe Locações Prime</option>
                  </select>
                </div>
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Gerente Responsável:</label>
                  <input
                    type="text"
                    value={managerName}
