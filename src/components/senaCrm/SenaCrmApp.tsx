...
export const SenaCrmApp: React.FC<SenaCrmAppProps> = ({ onBackToHome }) => {
  const [currentTab, setCurrentTab] = useState<SenaTab>("dashboard");
  ...
  const [rentalContracts, setRentalContracts] = useState<RentalContract[]>(INITIAL_RENTAL_CONTRACTS);
  ...
  const handleAddProperty = (newProp: Partial<Property>) => {
    const fullProp: Property = {
      ...
      images: newProp.images || ["https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=800&auto=format&fit=crop&q=80"],
  ...
  const handleUpdateVisitFeedback = (visitId: string, feedback: string, impression: any, status: any) => {
    setVisits((prev) =>
      prev.map((v) =>
  ...
  // Handlers: Lots
  const handleUpdateLotStatus = (devId: string, lotId: string, newStatus: LotStatus, buyerInfo?: any) => {
  ...
  const handleAddBroker = (newB: Partial<Broker>) => {
    const fullBroker: Broker = {
      ...
      commissionEarned: 0,
      conversionRate: 0,
      avatar: newB.avatar || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=300&auto=format&fit=crop&q=80",
    };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex font-sans antialiased selection:bg-amber-500 selection:text-slate-950">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 custom-scrollbar">
          ...
          {currentTab === "equipe-comissoes" && (
            <CommissionsModule commissions={commissions} />
          )}
