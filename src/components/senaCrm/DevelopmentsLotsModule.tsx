...
export const DevelopmentsLotsModule: React.FC<DevelopmentsLotsModuleProps> = ({
  ...
  const handleSaveLotStatus = (e: React.FormEvent) => {
    ...
    onUpdateLotStatus(currentDev.id, selectedLot.id, newStatusChoice, {
      reservedBy: newStatusChoice !== "Disponivel" ? buyerNameInput || "Cliente Reserva" : undefined,
    });
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400">{currentDev?.name} — {currentDev?.location}</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="p-2.5 bg-slate-950 rounded-xl border border-slate-800 text-right">
              <span className="text-[10px] text-slate-400 block uppercase font-bold">VGV Estimado</span>
              <span className="text-sm font-black text-amber-400">{formatCurrency(currentDev?.totalVgv || 0)}</span>
            </div>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[9px] text-slate-300/80 truncate">
                    {lot.reservedByClientName ? `👤 ${lot.reservedByClientName}` : `R$ ${pricePerM2}/m²`}
                  </div>
                </div>
              );
            })}
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <strong className="text-amber-400 font-bold text-sm">{formatCurrency(selectedLot.basePrice)}</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Valor por m²:</span>
                  <strong className="text-white">
                    {formatCurrency(selectedLot.areaM2 > 0 ? selectedLot.basePrice / selectedLot.areaM2 : 0)}/m²
                  </strong>
                </div>
                <div className="flex justify-between">
              ...
              {/* Status Change Form */}
              <form onSubmit={handleSaveLotStatus} className="pt-3 border-t border-slate-800 space-y-3 text-xs">
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Alterar Status do Lote:</label>
                  <select
                ...
                {newStatusChoice !== "Disponivel" && (
                  <div>
                    <label className="font-semibold text-slate-300 block mb-1">Nome do Comprador / Titular:</label>
                    <input
                      type="text"
            ...
            <div className="space-y-3 text-xs">
              <div>
                <div className="flex justify-between text-slate-300 mb-1">
                  ...
                  <span className="text-amber-400 font-bold">{formatCurrency(simDownPaymentVal)}</span>
                </div>
                <input
                  type="range"
                  min="10"
              ...
              <div>
                <div className="flex justify-between text-slate-300 mb-1">
                  ...
                  <span className="text-white font-bold">{simInterestRateMonthly}% a.m. + IPCA</span>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-slate-950 rounded-xl border border-slate-800 space-y-1.5">
                ...
                <span className="text-lg font-black text-emerald-400 block">
                  {formatCurrency(monthlyPaymentVal)} <span className="text-xs text-slate-400 font-normal">/mês</span>
                </span>
