...
export const VisitsModule: React.FC<VisitsModuleProps> = ({
  ...
  const handleFeedbackSubmit = (e: React.FormEvent) => {
    ...
    if (selectedVisitForFeedback) {
      onUpdateVisitFeedback(selectedVisitForFeedback.id, feedbackText, feedbackImpression, feedbackStatus);
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-400">Acompanhamento de visitas, confirmações e impressões do cliente</p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
          ...
          <div className="space-y-3">
            {todayVisits.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-6">Nenhuma visita agendada para hoje.</p>
            ) : (
              todayVisits.map((visit) => (
                ...
                >
                  ...
                  <div className="text-xs text-slate-300">
                    <strong className="text-amber-400 font-bold">{visit.propertyCode}</strong> • {visit.propertyTitle}
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-750 flex items-center justify-between text-xs">
                    <span className="text-slate-400 text-[11px]">Corretor: <strong className="text-slate-200">{visit.brokerName}</strong></span>
                    <button
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
          ...
          <div className="space-y-3">
            {upcomingVisits.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-6">Nenhuma próxima visita agendada.</p>
            ) : (
              upcomingVisits.map((visit) => (
                ...
                >
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-[10px] text-slate-400">{visit.brokerName.split(" ")[0]}</span>
                  </div>

                  <div className="text-xs text-slate-300 truncate">
                    <span className="text-amber-400 font-bold">{visit.propertyCode}</span> — {visit.propertyTitle}
                  </div>
                </div>
              ))
            )}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Histórico de Visitas & Feedbacks dos Clientes</h3>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {pastVisits.map((visit) => (
            <div key={visit.id} className="p-4 rounded-xl bg-slate-850/80 border border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-bold text-xs text-white">{visit.clientName}</span>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <form onSubmit={handleScheduleSubmit} className="space-y-3 text-xs">
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Cliente Interessado *</label>
                <input
                  type="text"
                  required
                  value={clientName}
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Imóvel a Visitar *</label>
                <select
                  value={propertyCode}
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Corretor Acompanhante *</label>
                <select
                  value={brokerName}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <button onClick={() => setSelectedVisitForFeedback(null)} className="text-slate-400 hover:text-white">✕</button>
            </div>
            ...
            <form onSubmit={handleFeedbackSubmit} className="space-y-3 text-xs">
              <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                ...
                <p className="text-slate-400 text-[11px]">{selectedVisitForFeedback.propertyCode} - {selectedVisitForFeedback.propertyTitle}</p>
              </div>
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Impressão Geral do Cliente</label>
                <select
                ...
                >
                  ...
                  <option value="Gostou mas achou caro">Gostou mas achou o valor acima do mercado</option>
                  <option value="Imóvel pequeno">Achou a metragem ou quantidade de quartos insuficiente</option>
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Comentários & Próximos Passos</label>
                <textarea
                  rows={3}
                  required
                  value={feedbackText}
