...
export const DashboardModule: React.FC<DashboardModuleProps> = ({
  ...
  const overdueRentals = rentalContracts.filter((c) => c.paymentStatus === "Em Atraso" || c.paymentStatus === "Vencido").length;
    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            ...
            <p className="text-xs lg:text-sm text-slate-400 max-w-2xl mt-1">
              Indicadores em tempo real para Vendas Alto Padrão, Locações Prime e Espelho de Vendas de Loteamentos.
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3.5">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col justify-between shadow-xs col-span-2">
          ...
          <div className="mt-3">
            ...
            <div className="text-[11px] text-slate-400 mt-1">
              Recebidas: <strong className="text-slate-200">{formatCurrency(totalCommissionsReceived)}</strong> (90%)
            </div>
          </div>
        </div>
        ...
        >
          ...
          <span className="text-[10px] text-emerald-400 font-medium">Compradores / Investidores</span>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <AlertTriangle className={`w-4 h-4 ${overdueRentals > 0 ? "text-rose-400" : "text-slate-500"}`} />
          </div>
          <div className={`text-2xl font-extrabold mt-2 ${overdueRentals > 0 ? "text-rose-400" : "text-slate-300"}`}>
            {overdueRentals}
          </div>
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400">Comparativo entre VGV Realizado e Meta Projetada</p>
            </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={vgvMonthlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <YAxis stroke="#94a3b8" fontSize={12} tickLine={false} tickFormatter={(val) => `R$${val}M`} />
                <Tooltip
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400">Classificação por VGV gerado e taxa de conversão</p>
            </div>
            <button
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-slate-400 truncate">{broker.team} • {broker.creci}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 text-right shrink-0">
                  <div>
                    ...
                    <span className="text-xs font-bold text-amber-300">{formatCurrency(broker.vgvTotal)}</span>
                  </div>
                  <div className="hidden sm:block">
                    <span className="text-[10px] text-slate-400 block">Conversão</span>
                    <span className="text-xs font-bold text-emerald-400">{broker.conversionRate}%</span>
                  </div>
          ...
          <div>
            ...
            <div className="space-y-2.5">
              <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs">
                ...
                <p className="text-slate-300 text-[11px] mt-1">Guilherme Leite aguarda resposta do Alphaville 1.</p>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-blue-500/10 border border-blue-500/20 text-xs">
                ...
                <p className="text-slate-300 text-[11px] mt-1">Vanessa & Thiago no Garden Vila Nova Conceição.</p>
              </div>

              <div className="p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-xs">
                <div className="flex items-center justify-between font-bold text-rose-300">
                  <span>Aluguel Vencido</span>
                  <span className="text-[10px] bg-rose-500/20 px-1.5 py-0.5 rounded">R$ 16.500</span>
                </div>
                <p className="text-slate-300 text-[11px] mt-1">Casa Comercial Harmonia (5 dias de atraso).</p>
              </div>
            </div>
          </div>
