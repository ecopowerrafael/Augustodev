...
export const ProposalsSalesModule: React.FC<ProposalsSalesModuleProps> = ({
  ...
  const [paymentDesc, setPaymentDesc] = useState("Entrada de recursos próprios + Financiamento Bancário.");
  ...
  const handleConfirmSaleClosure = (e: React.FormEvent) => {
    ...
    onApproveProposal(proposalToClose.id, {
      ...
      bankFinancing: salePaymentType === "Financiamento Bancário" ? { bank: saleBank, approvedAmount: finalPrice - proposalToClose.downPayment, status: "Aprovado" } : undefined,
  ...
  const handleConfirmCounter = (e: React.FormEvent) => {
    ...
    onSendCounterProposal(proposalForCounter.id, Number(counterPrice), counterNotes || "Contraproposta formal enviada ao proponente.");
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-400">Fluxo de análise de propostas, contrapropostas e emissão de contratos de venda</p>
        </div>
          ...
          <div className="bg-slate-950 p-1 rounded-xl border border-slate-800 flex items-center">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                activeTab === "proposals" ? "bg-amber-500 text-slate-950 font-bold" : "text-slate-400 hover:text-white"
              }`}
            >
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs">
                  <div className="md:col-span-2 p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
                    <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Imóvel Objeto da Negociação</span>
                    <p className="font-bold text-white text-sm">
                      <span className="text-amber-400 font-extrabold">{propDeal.propertyCode}</span> — {propDeal.propertyTitle}
                    </p>
                    <p className="text-[11px] text-slate-400">Corretor Responsável: {propDeal.brokerName}</p>
                  </div>

                  <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
                    <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Valor Anunciado</span>
                    <p className="text-slate-300 line-through font-semibold text-sm">{formatCurrency(propDeal.advertisedPrice)}</p>
                    <span className="text-[10px] text-slate-400 block">Tabela oficial</span>
                  </div>

                  <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
                    <span className="text-[10px] text-amber-400 uppercase font-bold tracking-wider">Valor Proposto</span>
                    <p className="text-amber-300 font-black text-base">{formatCurrency(propDeal.proposedPrice)}</p>
                    <span className={`text-[10px] font-bold ${difference < 0 ? "text-rose-400" : "text-emerald-400"}`}>
                ...
                <div className="p-3 bg-slate-850/60 rounded-xl border border-slate-800 text-xs">
                  <span className="font-bold text-slate-200 block mb-1">Condição de Pagamento Apresentada:</span>
                  ...
                  {propDeal.counterProposalPrice && (
                    <div className="mt-2 pt-2 border-t border-slate-750 text-amber-300">
                      <strong>Contraproposta do Proprietário:</strong> {formatCurrency(propDeal.counterProposalPrice)} — {propDeal.counterProposalNotes}
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-1.5 pt-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Histórico de Movimentações:</span>
          ...
          {sales.map((sale) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-amber-400 font-mono font-semibold">{sale.propertyCode}</p>
              </div>
              ...
              <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1.5 text-xs">
                ...
                <div className="flex justify-between pt-1 border-t border-slate-850">
                  ...
                  <span className="font-mono text-slate-300 text-[11px]">{sale.contractNumber}</span>
                </div>
              </div>
              ...
              <div className="pt-2 flex items-center justify-between text-xs">
                <div>
                  ...
                  <span className="text-base font-black text-amber-400">{formatCurrency(sale.finalSalePrice)}</span>
                </div>
                <div className="text-right">
                  <span className="text-[10px] text-slate-400 block">Comissão Gerada (6%)</span>
                  <span className="text-sm font-black text-emerald-400">{formatCurrency(sale.commissionTotal)}</span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <form onSubmit={handleCreateProposal} className="space-y-3 text-xs">
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Cliente Proponente *</label>
                <input
                  type="text"
                  required
                  value={clientName}
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Imóvel Pretendido *</label>
                <select
                  value={propertyCode}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Valor Proposto (R$) *</label>
                  <input
                    type="number"
                    required
                    value={proposedPrice}
                ...
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Valor da Entrada (R$)</label>
                  <input
                    type="number"
                    value={downPayment}
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Forma de Pagamento / Condições Especiais</label>
                <textarea
                  rows={2}
                  value={paymentDesc}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs space-y-1.5">
              ...
              <p className="text-amber-400 font-bold">
                Valor Final: {formatCurrency(proposalToClose.counterProposalPrice || proposalToClose.proposedPrice)}
              </p>
            ...
            <form onSubmit={handleConfirmSaleClosure} className="space-y-3 text-xs">
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Forma de Pagamento Definitiva</label>
                <select
              ...
              {salePaymentType === "Financiamento Bancário" && (
                <div>
                  <label className="font-semibold text-slate-300 block mb-1">Banco Concessor do Financiamento</label>
                  <input
                    type="text"
                    value={saleBank}
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Número do Contrato / CCV</label>
                <input
                  type="text"
                  required
              ...
              <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs">
                Ao confirmar, o status do imóvel será automaticamente alterado para <strong>Vendido</strong> e a comissão de <strong>6% ({formatCurrency((proposalToClose.counterProposalPrice || proposalToClose.proposedPrice) * 0.06)})</strong> será distribuída na aba de comissões.
              </div>
      ...
      {proposalForCounter && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
            <h3 className="text-base font-bold text-white">Registrar Contraproposta do Proprietário</h3>

            <form onSubmit={handleConfirmCounter} className="space-y-3 text-xs">
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Novo Valor da Contraproposta (R$) *</label>
                <input
                  type="number"
                  required
                  value={counterPrice}
              ...
              <div>
                <label className="font-semibold text-slate-300 block mb-1">Condições & Justificativa do Proprietário</label>
                <textarea
                  rows={3}
                  required
                  value={counterNotes}
